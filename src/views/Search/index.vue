<template>
  <div>
    ...
    <div class="search-header">
      <!-- 后退按钮 -->
      <van-icon name="arrow-left" color="white" size="0.48rem" class="goback" @click="$router.push('/layout')"/>
      <!-- 搜索组件 -->
      <van-search v-fofo  v-model="kw"  placeholder="请输入搜索关键词" background="#007BFF" shape="round" @input="inputFn"  @search="enterFn"/>
    </div>
    <!-- 搜索建议列表 -->
<div class="sugg-list" v-if="kw.length !== 0">
    <div class="sugg-item"  v-for="(str, index) in suggestList" :key="index" v-html="lightFn(str, kw)"   @click="clickFn(str)">
    </div>
</div>
<!-- 搜索历史 -->
...
export default {
  ...
  watch: {
    history () {
      const theSet = Array.from(new Set(this.history)) // 去重
  ...
  methods: {
    inputFn () {
      ...
      this.timer = setTimeout(async () => {
        ...
        this.suggestList = res.data.data.options[0] !== null ? res.data.data.options : '没有搜到！'
      }, 500)
    },
    // 输入框回车
    enterFn () {
